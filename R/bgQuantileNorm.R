...
function( bgfiles, normalizeto = 1:length(bgfiles) , threads=getOption("threads",1L) ){
	...
	if(unequal){
		cmdString <- paste("bash -c 'paste <(sort -T . -k4,4g",bgfiles,") <(shuf -n",fl,bgref," | sort -T . -k4,4g)' | awk '{print $1,$2,$3,$8}' OFS='\t' | sort -T . -k1,1 -k2,2n >",outnames)
		...
		bgref <- bgSort(bgref)
		cmdString <- paste("bash -c 'paste <(sort -T . -k4,4g",bgfiles,") <(sort -T . -k4,4g",bgref,")' | awk '{print $1,$2,$3,$8}' OFS='\t' | sort -T . -k1,1 -k2,3n >",outnames)
